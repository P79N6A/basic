 ...
 * 对初始化的思考:
 * (1)<clinit>方法是由编译器自动收集类中静态变量的赋值语句和静态代码块中的语句合并而产生,编译器收集的顺序取决于代码在源文件中出现的顺序
 ...
 * 对类加载器的思考:
 * 类加载器采用双亲委托机制加载类即当类加载器试图加载类时首先将加载任务委托给父类 依次递归 如果父类加载器可以完成类加载任务则返回 否则调用子类加载器去加载
 * BootStrap(启动类加载器):是用本地代码实现的类装入器，它负责将 <Java_Runtime_Home>/lib下面的类库加载到内存中
 * ExtClassLoader(扩展类加载器):是由 Sun 的 ExtClassLoader（sun.misc.Launcher$ExtClassLoader）实现 负责将< Java_Runtime_Home >/lib/ext或者由系统变量java.ext.dir指定位置中的类库加载到内存
 * AppClassLoader(系统类加载器):AppClassLoader（sun.misc.Launcher$AppClassLoader）实现 负责将系统类路径（CLASSPATH）中指定的类库加载到内存
 * 对java jvm垃圾回收器的思考:
 ...
 * 对运行时数据区的思考:
 * 运行时数据区包括 堆栈 方法区 堆 本地方法栈 程序计数器
 * 堆栈保存的数据包括:操作数栈 局部变量表 方法返回地址 指向常量池的引用 附加信息
 * 方法区(永久代):类名 类信息 方法信息 常量 类变量
 * 堆:存储对象本身和数组
 * 程序计数器:存储cpu当前需要执行的指令
 */
public class SupAndSubClassDemo {
    public static void main(String[] args) {
        /**
         * 对new SubClass()执行过程的思考:
         * 当实例化一个类的对象时JVM首先会对该类的父类进行初始化即首先初始化父类的静态代码块和静态变量(按代码出现的顺序执行 静态属性只初始化一次)
         * 然后对子类初始化即初始化子类的静态代码块和静态变量(按代码出现的顺序执行 静态属性只初始化一次) 然后调用子类构造方法
         * 由于父类构造方法执行优先于子类构造方法 继而先执行父类构造器 由于类的实例域赋值和block块执行总是优于构造方法的执行  继而先初始化父类的实例域和block
         * 然后返回执行构造方法里的代码 当父类完成所有初始化工作后 子类将进行实例域赋值和执行block块的操作 最后返回执行构造方法里的代码
         */
        System.out.println("contant is "+ContantUtils.contant);
        new SubClass();
    }
}
